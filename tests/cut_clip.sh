...
END=$(($START + $FRAME - 1))
ffmpeg -i $ORIGPATH -s ${small_width}x${small_height} -vcodec rawvideo -f rawvideo -pix_fmt yuv420p -vf select="between(n\,$START\,$END),setpts=PTS-STARTPTS" -vsync vfr $SMALPATH -y # 原始视频缩放后转rawvideo
